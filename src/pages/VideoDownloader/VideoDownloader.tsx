...
const VideoDownloader = () => {
    ...
    const getPlatformFromUrl = (url: string) => {
        if (url.includes("youtube.com") || url.includes("youtu.be")) {
            ...
            if (selectedOption !== "auto" && selectedOption === "format") setSelectedOption("auto");
            return "instagram";
        }
        return "unknown";
    };
    ...
    const handleFetchVideoDetails = async () => {
        ...
        if (platform === "unknown") {
            showToast("error", "Invalid URL", "Please enter a valid YouTube or Instagram URL.");
            return;
        }
        if (platform === "instagram" && selectedOption === "audio") {
            showToast("error", "Not Supported", "Audio-only download is not available for Instagram Reels.");
            return;
        }
        setLoading(true);
        ...
        try {
            ...
            switch (selectedOption) {
                case "auto": {
                    ...
                    await DownloadVideoAndMerge(formatDetails, details, mergeStreams, platformUrl);
                    toast.dismiss("status-toast");
                    showToast(
                        "success",
